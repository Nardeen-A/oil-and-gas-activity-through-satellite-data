proj.path <- getwd()
data <- read.csv(file.path(proj.path, "output", "data", "ml_oilgas_panel_1km_2018_2024.csv"))
...
# ---------------------------------------------------------
plot_dt <- copy(x[year == 2020 & month == 5])
...
theme_map_polished <- function() {
  theme_void() +
    theme(
      ...
      plot.subtitle = element_text(size = 11.5, hjust = 0.5, color = "grey35"),
...
make_pretty_map <- function(sf_dt, varname) {
  ...
  subtitle_txt <- "Kurdistan Region + Kirkuk \u00b7 January 2020 \u00b7 1 km grid"
